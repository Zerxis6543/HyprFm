...
// Zone label — only shown when hp < 100
// Coordinates are in the SVG's 200×223 body space.
// x2/y2 extend INTO the side-slot area; SVG overflow:visible lets them show.
function BodyFigure({ health }: { health: number }){
  const glow = withAlpha(zoneColor(health), 0.28)
  return (
    // ── SVG coordinate system ──────────────────────────────────────────────
    // viewBox "0 0 200 223" — body fills the ENTIRE box.
    // The container div is sized purely by height (fills e-top row height).
    // Width is auto → determined by the aspect ratio.
    // overflow:visible lets zone labels extend left/right over the slot columns.
    // Zone polygon coords: scaled from 80×215 original → 200×223
    //   sx=2.5, sy=1.037
    <svg
      viewBox="0 0 200 223"
...
export function EquipmentPanel() {
  ...
  return (
    <div className="equip-panel">
      ...
      {/*
        Layout strategy:
          - e-top uses align-items:stretch so the center column grows to the
            full height of whichever side column is taller.
          - e-center is a flex column that fills that height via flex:1/height:100%.
          - e-figure fills all remaining space above the HP readout.
          - The SVG inside uses width:100% height:100% and overflow:visible
            so it fills the box exactly, and zone labels bleed over side cols.
      */}
      <div className="e-top">
      ...
      <style>{`
        ...
        .e-top {
          ...
          gap: 8px;
          /* stretch so center column fills the same height as side columns */
          align-items: stretch;
        }
        ...
        .e-center {
          ...
          /* overflow:visible lets SVG labels extend into side columns */
          overflow: visible;
          /* No padding — figure should touch the edges of the cell */
        }

        /* ── Figure container ── */
        /* Fills all height above the HP text.
           width:100% + height:100% on the SVG inside then scales to fill it.
           The SVG viewBox is 200:223 (≈0.897) so the figure is taller than wide — 
           in a cell that's wider than it is tall the figure will be height-constrained
           and correctly proportioned. */
        .e-figure {
          flex: 1;
          width: 100%;
          min-height: 0;
          /* Center the SVG horizontally within the cell */
          display: flex;
          align-items: stretch;
        ...
        .e-figure svg {
          /* SVG fills the container height; width adjusts to maintain aspect ratio */
          height: 100%;
          width: auto;
        }
        ...
        .e-slot-label {
          ...
          border-radius: 3px 3px 0 0; padding: 3px 6px; white-space: nowrap;
        }
        .e-slot {
